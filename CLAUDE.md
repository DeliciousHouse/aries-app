...
- `ARIES_AUTO_APPROVE_MARKETING_PIPELINE=1` — when Hermes emits `requires_approval` for strategy/production/**publish**, Aries synthesizes an `ai-orchestrator` approval from the callback handler and resumes the pipeline without a human click — including the publish gate, so content goes to live social media with **no human review** (the only refusal is when Hermes itself flags the run not-ready: preflight failed / `publish_ready=false` / review rejected). Closes the autonomous E2E loop. Process-wide — affects ALL tenants in this container (no per-tenant control). **Default OFF (`:-0`) in `docker-compose.yml` — safe-by-default so a missing `.env` line cannot silently enable autopublish.** Set to `1` in `.env` only for a single-tenant autonomous deployment you fully control. With it OFF, jobs hold at `awaiting_approval`; pair with `ARIES_REAPER_ENABLED=0` or the stale-run reaper kills them before a human can approve.
