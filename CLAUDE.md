...
- `scripts/automations/` — holds two long-lived sidecar workers, each self-scheduling via `setInterval` and each run as a single-replica `restart: unless-stopped` service in `docker-compose.yml` (both point `APP_BASE_URL` at the in-network `http://aries-app:3000` to skip the public DNS+TLS round-trip; no external cron required):
