...
- Run `OPENCLAW_GATEWAY_URL=http://127.0.0.1:3456 python scripts/smoke-openclaw-media-gateway.py` for a local gateway smoke test. Add `LOBSTER_SMOKE_SKIP_VIDEO=1` to avoid a long video render while still exercising the live `image_generate` path.
