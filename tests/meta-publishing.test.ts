...
test('publishToMetaGraph: a final Facebook 429 crosses the non-idempotent boundary only once', async () => {
  process.env.OAUTH_TOKEN_ENCRYPTION_KEY = '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!';
  const restoreQuery = installOauthQueryFixture({
    access_token_enc: encryptOAuthSecret('page-token'),
    connection_id: 'conn_fb_final_429',
    external_account_id: 'page_final_429',
  });
  ...
  try {
    const caught = await publishToMetaGraph({
      tenantId: '12',
      provider: 'facebook',
      content: 'Retry only under a new attempt token',
      mediaUrls: [],
      fetchImpl: (async (input: RequestInfo | URL) => {
        assert.match(String(input), /\/page_final_429\/feed$/);
        finalPublishCalls += 1;
    ...
    assert.ok(caught instanceof MetaPublishError);
    assert.equal(caught.code, 'graph_rate_limited');
    assert.equal(caught.retryable, true, 'an explicit rejection may be claimed later under a new token');
    assert.equal(caught.outcomeUnknown, false);
    assert.equal(finalPublishCalls, 1, 'the same call/token must never replay a final provider POST');
  } finally {
    restoreQuery();
  }
});
