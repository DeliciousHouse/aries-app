  ...
  await withRuntimeEnv(async () => {
    ...
    try {
      ...
      assert.equal(calls[0].body.callback_url, 'https://aries.example.com/api/internal/hermes/runs');
       ...
       assert.equal(typeof callbackAuth, 'object');
       assert.notEqual(callbackAuth, null);
       const callbackAuthRecord = callbackAuth as Record<string, unknown>;
       assert.equal(callbackAuthRecord.type, 'internal_api_secret_bearer');
       assert.equal(callbackAuthRecord.secret_ref, 'INTERNAL_API_SECRET');
       assert.match(String(callbackAuthRecord.callback_token), /^[0-9a-f]{64}$/);
