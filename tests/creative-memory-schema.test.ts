...
test('Creative Memory DDL includes provenance joins and indexes', () => { assert.match(ddl,/FOREIGN KEY \(tenant_id, style_card_id\) REFERENCES style_cards\(tenant_id, id\)/); assert.match(ddl,/FOREIGN KEY \(tenant_id, creative_analysis_id\) REFERENCES creative_analyses\(tenant_id, id\)/); assert.match(ddl,/idx_campaign_learning_labels_tenant_label_created/); const labelsStart = ddl.indexOf('CREATE TABLE IF NOT EXISTS campaign_learning_labels'); const labelsEnd = ddl.indexOf('CREATE INDEX IF NOT EXISTS idx_creative_assets_tenant_status', labelsStart); const labelsSection = ddl.slice(labelsStart, labelsEnd); assert.doesNotMatch(labelsSection,/ON DELETE SET NULL/); assert.match(labelsSection,/CHECK \(prompt_recipe_id IS NOT NULL OR generated_asset_id IS NOT NULL OR marketing_job_id IS NOT NULL\)/); });
