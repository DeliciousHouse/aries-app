...
test('collectProductionReviewArtifacts emits one video artifact per rendered variant', async () => {
  ...
  try {
    ...
    await writeJson(path.join(tempRoot, 'tenant-video-artifacts', runId, 'video_render.json'), {
      type: 'video_render',
      run_id: runId,
      video_assets: {
        platform_contracts: [
...
test('collectProductionReviewArtifacts skips rate-limited video variants so review can continue without broken previews', async () => {
  ...
  try {
    ...
    await writeJson(path.join(tempRoot, 'tenant-video-rate-limited-artifacts', runId, 'video_render.json'), {
      type: 'video_render',
      run_id: runId,
      video_assets: {
