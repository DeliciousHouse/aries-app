...
test('one row throwing never kills the loop — the rest still process', async () => {
  ...
  const report = await runFeedbackRetrySweep(FAKE_POOL, config(), {
    ensureTable: async () => {},
    claim: async () => [row('a'), row('boom'), row('c')],
    sync: async (syncable) => {
...
test('rows keep their loaded attempts/ticket key through rowToSyncable (attach-only path)', async () => {
