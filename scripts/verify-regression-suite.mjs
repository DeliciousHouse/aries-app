...
const steps = [
  ...
  },
  {
    ...
    // AA-129 backlog items 10 + 12. ITEM 12: the unused
    // `insights_llm_calls` cost table is superseded by `task_execution_log`
    // (AA-159). Production deploys execute init-db, so that path performs the
    // idempotent drop; the migration is only its schema-history mirror. The
    // live-Postgres regression seeds a row before running db:init, avoiding any
    // unsupported claim about the table's historical production row count.
    ...
    // The rendered copy is driven off the capability matrix, so it follows the
    // facts if LinkedIn ever gains support; 'all' keeps the generic message.
