...
// Matches only IDs generated by createExecutionRunRecord: arun_<uuid-v4>
const ARIES_RUN_ID_RE = /^arun_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;
...
export function parseHermesRunCallbackPayload(value: unknown): HermesRunCallbackPayload | null {
  ...
  if (!eventId || !ARIES_RUN_ID_RE.test(ariesRunId) || !isCallbackStatus(status)) {
    return null;
  }
