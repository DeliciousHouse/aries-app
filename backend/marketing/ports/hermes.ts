...
const PRODUCTION_EXECUTION_CONTRACT =
  'PRODUCTION STAGE EXECUTION CONTRACT: When the input contains "Production context (N images requested)", you MUST return BOTH content_package[] AND artifacts.creative_assets[]. One without the other is incomplete and will fail downstream publish. (A) Call the `image_generate` tool for every image listed — do not return JSON until every image listed has either a completed image_generate result or a recorded artifacts.errors[] entry. IMAGE PROMPT DISCIPLINE: the `prompt` argument you pass to image_generate MUST contain ONLY the visual description of that image (your rendered visual_prompt) — NEVER the workflow header, stage context, JSON schemas, or copywriting instructions. The image model renders exactly what you send it, and non-visual context makes the render fail. IMAGE RETRY: if image_generate fails or returns success:false, retry the SAME call up to 2 more times before giving up on that image — a transient failure usually succeeds on retry. A requested image with neither a creative_asset nor an artifacts.errors[] entry is a stage failure. (B) Build content_package[] with one entry per post: {post_number, theme, hook, body, cta, hashtags (array of 3-6 relevant hashtags), platforms, format, visual_prompt}. The Nth creative_asset corresponds to the Nth content_package post via post_number. content_package carries the post COPY (caption text, hooks, hashtags). creative_assets carries the rendered IMAGES. Return output:[{stage:"production", content_package:[{post_number:1, theme:"...", hook:"...", body:"...", cta:"...", hashtags:["#tag1","#tag2","#tag3"], platforms:["instagram","facebook"], format:"single_image", visual_prompt:"..."},...], artifacts:{creative_assets:[{assetId:"img_1", type:"generated_image", path:<absolute path returned by image_generate>, prompt:<the rendered visual prompt>, placement:<which post number>}, ...], errors:[]}}]. If image_generate still fails after retries, record it in artifacts.errors[] and continue.';
...
function buildWeeklyResearchInstructions(workflowKey: string, last30daysRequired = true): string {
  return [
    ...
    ...LAST30DAYS_GUIDANCE,
    ...(last30daysRequired
      ? [WEEKLY_RESEARCH_LAST30DAYS_MANDATE, ...WEEKLY_RESEARCH_AGENT_REACH_GUIDANCE]
      : []),
