...
import { queryProductionCreativeAssets } from './production-assets-query'
...
async function buildSocialContentJobContentInternal(context: SocialContentBuildContext): Promise<MarketingDashboardContentInternal> {
  ...
  // B1 safety net — supplement from DB when the publish-stage payload yielded
  // no posts AND no assets (e.g. Hermes publish stage emitting a strategy-shaped
  // placeholder). STRICT GATE: only fires when BOTH arrays are empty; a healthy
  // job with a real publish payload is byte-identical to today (no double-count).
  //
  //   empty payload → DB creative_assets → preview thumbnail assets
  //   empty payload → DB posts status counts → counts.posts / readyToPublish /
  //                   scheduled / live reflect reality instead of 0
  //
  // Does NOT reconstruct publishItems, cadence, provenance, title, or summary.
  if (posts.length === 0 && assets.length === 0) {
    const tenantId = context.runtimeDoc.tenant_id
    const jobId = context.jobId
    // Guard: skip DB supplementation when tenantId is not a valid positive
    // integer. Tests use string IDs like 'tenant-hermes'; attempting pool.query
    // for those would either error (no DB) or produce a PG type-cast error, and
    // the resulting console.warn would bleed into warning-count assertions in
    // concurrent test runs. This mirrors the NaN guards in
    // queryProductionCreativeAssets and countPublishedPostsForJob.
    const tenantIdNum = Number(tenantId)
    if (Number.isFinite(tenantIdNum) && tenantIdNum > 0) try {
      // (a) DB creative assets → preview thumbnail entries.
      const dbAssets = await queryProductionCreativeAssets(tenantId, jobId)
      for (const row of dbAssets) {
        // served_asset_ref is `/api/internal/hermes/media/<uuid>`. Prefer that;
        // fall back to `/api/internal/hermes/media/<source_asset_id>` for rows
        // ingested before the self-referential INSERT was shipped.
        const previewUrl = row.served_asset_ref ?? (row.source_asset_id ? `/api/internal/hermes/media/${row.source_asset_id}` : null)
        ...
        assets.push({
          ...
          postId,
          jobId,
          type: isVideo ? 'video_ad' : 'image_ad',
          ...
          destinationUrl: null,
          previewUrl,
          thumbnailUrl: isVideo ? null : previewUrl,
          contentType: isVideo ? 'video/mp4' : 'image/png',
          filePath: null,
          status: 'ready_to_publish' as MarketingDashboardItemStatus,
          createdAt: context.status.updatedAt,
          relatedPostIds: [],
          relatedPublishItemIds: [],
          provenance: {
            sourceKind: 'creative_output' as MarketingDashboardSourceKind,
            sourceStage: 'production' as MarketingStage,
            sourceRunId: context.runtimeDoc.stages.production.run_id,
            sourcePath: null,
            isDerivedSchedule: false,
            isPlatformNative: false,
      ...
      // Refresh counts.imageAds / videoAds to include the supplemented DB assets.
      if (dbAssets.length > 0) {
        counts.imageAds = assets.filter((a) => a.type === 'image_ad').length
        counts.videoAds = assets.filter((a) => a.type === 'video_ad').length
      }

      // (b) DB posts status counts. Map published_status →
      //   approved       → readyToPublish
      //   scheduled      → scheduled
      //   published      → live
      //   (others ignored for counts purposes)
      const postCountRows = await pool.query<{ published_status: string; cnt: string }>(
        `SELECT published_status, COUNT(*) AS cnt
           FROM posts
          WHERE job_id = $1
            AND tenant_id = $2
          GROUP BY published_status`,
        [jobId, tenantIdNum],
      )
      if (postCountRows.rowCount && postCountRows.rowCount > 0) {
        let dbPosts = 0
        let dbReadyToPublish = 0
        let dbScheduled = 0
        let dbLive = 0
        for (const r of postCountRows.rows) {
          const n = parseInt(r.cnt, 10) || 0
          dbPosts += n
          if (r.published_status === 'approved') dbReadyToPublish += n
          else if (r.published_status === 'scheduled') dbScheduled += n
          else if (r.published_status === 'published') dbLive += n
        }
        counts.posts = dbPosts
        counts.readyToPublish = dbReadyToPublish
        counts.scheduled = dbScheduled
        counts.live = dbLive
      }
    } catch (err) {
      // Non-fatal: B1 supplementation failure must never break the dashboard.
      console.warn('[dashboard-content] B1 safety net failed — returning payload-only view', {
        jobId: context.jobId,
