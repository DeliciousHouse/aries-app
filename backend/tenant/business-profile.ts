...
export type BusinessProfileView = {
  ...
  /**
   * S6-1/AA-114: the canonical goal the operator CHOSE. Null means "not set" —
   * which the S1-5 confirm chip surfaces rather than hiding behind a guess.
   * `primaryGoal` stays alongside it as the descriptive free text that feeds the
   * Hermes brand prompts; the two are not interchangeable.
   */
...
type BusinessProfileUpdateInput = {
  ...
  primaryGoal?: string | null;
  /**
   * S6-1/AA-114: the canonical goal from the select. `undefined` means the
   * caller did not touch it (legacy/API writers); `null` explicitly clears it.
   */
  goalType?: GoalType | null;
  launchApproverUserId?: string | null;
  launchApproverName?: string | null;
...
function normalizeBusinessProfileRecord(
  ...
  return {
    ...
    // S6-1/AA-114: honor a PERSISTED key. This previously re-derived from the
    // free text on every load, which silently discarded an operator's explicit
...
function buildBusinessProfileView(input: {
  ...
  return {
    ...
    businessType: effectiveBusinessType,
    primaryGoal: effectivePrimaryGoal,
    // S6-1/AA-114: the operator's stored choice, surfaced as-is. Deliberately
    // NOT derived from effectivePrimaryGoal here — an unset key must read as
    // unset so the confirm chip can ask, rather than as a confident guess.
