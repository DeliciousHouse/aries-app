...
export function createResearchAgent(input: CreateAgentInput): ProvisionedAgent {
  ...
  const workspacePath = workspacePathFromConfig(projectRoot, input.tenantId, workspaceConfig.parsed, 'research');
  ...
  return {
    ...
    agent_type: 'research',
    agent_id: `${input.tenantId}-research-agent`,
    workspace: 'research',
    workspace_path: workspacePath,
    capabilities: ['workspace.read', 'workspace.write', 'analysis.summarize', 'analysis.compare'],
    policy: {
      deterministic: true,
