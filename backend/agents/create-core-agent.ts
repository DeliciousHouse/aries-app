...
export function createCoreAgent(input: CreateAgentInput): ProvisionedAgent {
  ...
  const workspacePath = workspacePathFromConfig(projectRoot, input.tenantId, workspaceConfig.parsed, 'core');
  ...
  return {
    ...
    agent_type: 'core',
    agent_id: `${input.tenantId}-core-agent`,
    workspace: 'core',
    workspace_path: workspacePath,
    capabilities: ['workspace.read', 'workspace.write', 'tenant.lifecycle.observe'],
    policy: {
      deterministic: true,
