...
const moduleConfigs: Record<ModuleId, ModuleConfig> = {
  ops: {
    ...
    tabs: [
      ...
      { id: 'standups', label: 'Standups', icon: 'clipboard-list' },
    ],
  },
  brain: {
    id: 'brain',
...
function StandupsView({ standups }: { standups: AriesOsStandupEntry[] }) {
  const [expandedId, setExpandedId] = useState<string | null>(standups[0]?.id ?? null);

  if (standups.length === 0) {
    return (
      <div className={styles.contentGrid}>
        <section className={styles.panel}>
          <EmptyState label="No standup artifacts" copy="No persisted standup transcripts were found in /home/node/.openclaw/projects/shared/teams/meetings." />
        </section>
      </div>
    );
  }
    ...
    <div className={styles.contentGrid}>
      <section className={styles.panel}>
        <div className={styles.panelHeader}>
          <div>
            <p className={styles.cardKicker}>PM Board Standups</p>
            <h3 className={styles.panelTitle}>{standups.length} standup transcript{standups.length !== 1 ? 's' : ''}</h3>
          </div>
        </div>
        <div className={styles.detailsStack}>
          {standups.map((standup) => {
            ...
            return (
              <details
                ...
                open={isExpanded}
                onToggle={(e) => {
                  if ((e.target as HTMLDetailsElement).open) setExpandedId(standup.id);
                }}
              >
                <summary>
                  <span>
                    <p className={styles.cardKicker}>{standup.date}</p>
                    <strong>{standup.title}</strong>
                  </span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                    <span className={styles.statPill} style={{
                      background: standup.status === 'complete' ? 'rgba(52, 211, 153, 0.15)' : standup.status === 'failed' ? 'rgba(251, 113, 133, 0.15)' : 'rgba(251, 191, 36, 0.15)',
                      color: standup.status === 'complete' ? '#6ee7b7' : standup.status === 'failed' ? '#fda4af' : '#fcd34d',
                    }}>{standup.status}</span>
                ...
                <div className={styles.detailsBody}>
                  <div style={{ display: 'grid', gap: 12, marginBottom: 16 }}>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {standup.boardPath && <span className={styles.pathPill}>Board: {standup.boardPath}</span>}
                      <span className={styles.pathPill}>Transcript: {standup.transcriptPath}</span>
                      {standup.reportDirectory && <span className={styles.pathPill}>Reports: {standup.reportDirectory}</span>}
                    </div>

                    {standup.preview && (
                      <div style={{ padding: '12px 14px', borderRadius: 16, background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.06)' }}>
                        <p style={{ margin: 0, fontSize: '0.78rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)' }}>Top summary</p>
                        <p className={styles.copy} style={{ marginTop: 8 }}>{standup.preview}</p>
                      </div>
                    )}
                  </div>

                  {standup.chiefs.length > 0 && (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 10, marginBottom: 16 }}>
                      {standup.chiefs.map((chief) => (
                        <div key={chief.chiefId} style={{ padding: '12px 14px', borderRadius: 16, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                          <p style={{ margin: 0, fontSize: '0.9rem', fontWeight: 600, fontFamily: 'var(--font-title)' }}>{chief.title}</p>
                          <p style={{ margin: '4px 0 0', fontSize: '0.75rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)' }}>{chief.reportStatus}</p>
                          <div style={{ marginTop: 10, fontSize: '0.88rem', color: 'rgba(234,243,255,0.6)', display: 'grid', gap: 4 }}>
                            <p style={{ margin: 0 }}><span style={{ color: 'rgba(255,255,255,0.35)' }}>Task:</span> {chief.activeTaskId || 'None recorded'}</p>
                            <p style={{ margin: 0 }}><span style={{ color: 'rgba(255,255,255,0.35)' }}>Blockers:</span> {chief.blockedCount}</p>
                            <p style={{ margin: 0 }}><span style={{ color: 'rgba(255,255,255,0.35)' }}>Routing:</span> {chief.needsRouting ? 'Needed' : 'Clear'}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <details style={{ borderRadius: 16, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.025)', overflow: 'hidden' }}>
                    <summary style={{ listStyle: 'none', cursor: 'pointer', padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
                      <span style={{ fontSize: '0.9rem', fontWeight: 600 }}>Read full transcript</span>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: '0.82rem', color: 'rgba(234,243,255,0.6)' }}>
                        Expand <ChevronDown size={14} />
                      </span>
                    </summary>
                    <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', padding: '14px' }}>
                      <MarkdownView markdown={standup.transcriptBody} />
                    </div>
                  </details>
                </div>
              </details>
...
export default function AriesOsShell({ data }: { data: AriesOsWorkspaceData }) {
  const initialTab: TabId = data.initialView === 'standups' ? 'standups' : 'dashboard';
      ...
      <div className={styles.shellInner}>
        ...
        <section className={styles.surface}>
          ...
          <main className={styles.mainPane}>
            ...
            {activeModule === 'ops' && activeTabId === 'standups' ? (
              <StandupsView standups={data.ops.standups} />
            ) : null}
