...
// AA-114: the presets now come from the SHARED list, where each carries the
// canonical goal it means explicitly. Previously these labels were mapped back
// to a goal by keyword, which is how gap A6a's "Increase social media presence"
// ended up on brand awareness instead of audience growth — the exact goal whose
// metric (SUM of followers_delta) is what #964's prompts now optimise for.
 ...
 *
 * AA-114 retired the keyword chain that used to live here (lead/sell/social/quiz
 * substring tests over the free text). It guessed, and it disagreed with the
 * backend's own families. The stored canonical `goalType` is now the answer when
 * there is one; otherwise the stored text is shown back verbatim, which is what
 * a custom "Other" goal always was.
 */
