...
const InviteAcceptForm: React.FC<InviteAcceptFormProps> = ({ email, onSignIn, onSubmit, isLoading }) => {
  ...
  const canSubmit = passwordMeetsPolicy && passwordsMatch;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to set your password. Try again.');
    }
  };
  ...
  return (
    <div className="auth-container animate-in fade-in duration-1000" style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="flex flex-col items-center mb-6 text-center">
        ...
        <h1 className="text-[26px] font-medium text-white tracking-[0.25em] mb-[20px] uppercase leading-none pl-[0.25em]" style={{ marginBottom: '20px' }}>
          JOIN THE TEAM
        </h1>
        <p className="text-white italic text-[18px] tracking-wide font-normal mt-[20px] opacity-90 px-4">
          Set a password to finish joining the workspace
        </p>
        {email ? (
          <p className="text-white/70 text-[14px] mt-3 px-4 break-all">
            You're accepting an invite for <span className="font-semibold text-white">{email}</span>
          </p>
        ) : null}
      </div>
      ...
      <div className="auth-card animate-in fade-in zoom-in-95 duration-500">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <label htmlFor="invite-new-password" className="auth-label">New Password</label>
            <input
              ...
              aria-describedby={passwordError ? 'invite-new-password-error' : undefined}
            />
            {passwordError ? (
              <p id="invite-new-password-error" role="alert" className="text-sm text-red-300">
                {passwordError}
              </p>
            ) : null}
          </div>
          <div className="space-y-2">
            <label htmlFor="invite-confirm-password" className="auth-label">Confirm Password</label>
            <input
              ...
              aria-describedby={confirmPasswordError ? 'invite-confirm-password-error' : undefined}
            />
            {confirmPasswordError ? (
              <p id="invite-confirm-password-error" role="alert" className="text-sm text-red-300">
                {confirmPasswordError}
              </p>
            ) : null}
          </div>
          ...
          {error && (
            <div
              ...
              role="alert"
              style={{ padding: '16px', marginBottom: '32px', borderLeft: '4px solid #EF4444' }}
            >
              <div
          ...
          <button type="submit" disabled={submitDisabled} className="auth-primary-button">
      ...
      <div className="text-center font-medium text-[15px]" style={{ marginTop: '20px', color: '#FFFFFF' }}>
        <p>
          ...
          <button
            type="button"
            onClick={onSignIn}
            className="underline underline-offset-4 decoration-white/40 hover:decoration-white transition-all ml-1 font-bold"
          >
            Sign In
          </button>
        </p>
