...
- **Comment classification** — `backend/insights/sync/classify-comments.ts` (raw Hermes run, bounded 40/batch/tick, isolated to `legErrors`), 8-case unit test, shipped default OFF; compose default flipped ON 2026-07-16 (S1-11, see changelog).
...
| # | Gap | Evidence | Effort |
|---|-----|----------|--------|
| B1 | **Comment classification OFF in prod** — ✅ resolved 2026-07-16: compose default now `ARIES_COMMENT_CLASSIFICATION_ENABLED:-1` on the `aries-insights-sync-worker` service (S1-11, #847). With it off: Conversations 0% positive + empty asks panel, goal `lead_generation` reads 0, attention lead counts 0, top per-post sentiment null, trends sentiment 0 — five surfaces. Code complete + tested; needs Hermes creds verified in the **worker** env, flag flip, then watch `insights_sync_runs` + one classified batch. | flags | XS (ops) |
