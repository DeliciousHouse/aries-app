...
| A2 | **No data-freshness surfacing anywhere.** `insights_accounts.last_sync_at` and `insights_sync_runs` are write-only; integrations cards hardcode `last_synced_at: null` (`app/api/integrations/handlers.ts:128,214`); `/insights` renders no "data as of" stamp. A silently failing sync looks like an engagement plateau. | observability | S+M |
...
| A6 | **Silent goal misclassification:** free-text `primary_goal` (4 writers, all free-form) → `normalizeGoal` keyword guess defaulting every miss to `brand_awareness` with zero signal (`goal-snapshot-builder.ts:69-83`); shipped onboarding preset "Increase social media presence" verifiably lands on the wrong goal. Goal edits stale-cached up to 1h (nothing invalidates the `insights_narratives` goal row on profile save). `no_goal` empty state says "Go to Settings" with no link. | product/data | M+S+XS |
...
| C2 | **`aries_post_id` has no production writer** — attribution impossible; #785 rescoped Activity/Top to all-channel counts as a workaround. Stamp at Meta-publish/scheduled-dispatch time (`platform_post_id` join), backfill, then reintroduce attribution views **with** the backfill (re-adding the filter without it re-empties the sections). | publish+sync | M |
...
*Budget note: S4 lists 10.0d of tagged work (S+M+M+M+S+M = 1+2.5+2.5+2.5+1+2.5 = 12 — over); S4-3 is the designated cut (slips to Sprint 5's cut-line slot or Sprint 6), and S4-6 is the second cut if both squeeze. Do not cut S4-4 after S4-2 has landed, and never cut S4-5.*
