...
- 2026-08-02 — **S3-4 / F1a done — weekly-results plan rebased** against as-built insights (`docs/plans/2026-06-01-weekly-results-next-action.md`, rebase section). Corrects five stale premises, the load-bearing one being that the draft routed engagement through the `#513` gate whose frozen SQL mismatches the landed schema on six axes (roadmap B2) — so it gated best/weakest behind a flag that cannot be flipped, while the data sits unflagged in `insights_post_metrics_daily`. Best/weakest now ranks over the shipped read model via `LATEST_POST_METRICS_LATERAL` (S2-1), reusing `top-snapshot-builder.ts`. **MVP slice agreed:** phases A/B/C/D.2/F ship as S5-1; the Honcho promotion loop (D.1/D.3 + E) defers behind the B2 repair, since that is the only part still blocked. S5-1 unblocked.
- 2026-08-02 — **S1-10 / F6 RATIFIED — analytics surface consolidation.** `/insights` is the **primary** analytics surface: the cross-channel dashboard operators are routed to by default, and the only analytics surface that receives further shell investment. `/dashboard/analytics` is **retained, not retired**, in a **narrowed** role — the **per-platform drill-down** reached from `/insights` for single-platform depth (per-platform metric cards, trend chart, per-post table, capability-gated columns per `backend/insights/platforms/capabilities.ts`). It is explicitly no longer a second general-purpose analytics dashboard. Retention (rather than retirement) is load-bearing for S2-1, whose acceptance requires the two surfaces to render **agreeing totals** — a drill-down that disagrees with its parent is the same trust bug as two competing nav entries. Scope of this decision is the ratification only: the route stays, and its top-level nav entry (`frontend/app-shell/routes.ts`, `id: 'analytics'`) stays, until **backlog item 9** executes the convergence. Shell investment is unblocked as of this entry.
- 2026-08-13 — **Backlog item 9 / S1-10 F6 consolidation EXECUTED**, across four PRs under AA-229: `/dashboard/analytics` narrowed to a per-platform drill-down reached from `/insights`, with its top-level nav entry removed (#995, PR1); Section 6 (Top Performing Content) gained real best/weakest-post ranking, taking that responsibility off the weekly-results report (#997, PR2a); the weekly-results report relocated into `/insights` as **Section 10 — Weekly Recap** (`GET /api/insights/weekly-recap`, `backend/insights/weekly-recap/**`) and the standalone `/dashboard/results` page retired to a bare redirect to `/insights` (#998, PR2b); and every remaining stale doc/comment reference to the old `/dashboard/results` location and `backend/marketing/weekly-results-*` module path was re-pointed (PR2c, this entry). `tests/insights-dashboard-ui.test.ts`'s dual-top-level-nav pin moved with PR1 per the note on item 9 above.
...
- **Comment classification** — `backend/insights/sync/classify-comments.ts` (raw Hermes run, bounded 40/batch/tick, isolated to `legErrors`), 8-case unit test, shipped default OFF; compose default flipped ON 2026-07-16 (S1-11, #847 — recorded in this doc's changelog above).
...
| # | Gap | Evidence | Effort |
|---|-----|----------|--------|
| B1 | **Comment classification OFF in prod** — ✅ resolved 2026-07-16: compose default now `ARIES_COMMENT_CLASSIFICATION_ENABLED:-1` on the `aries-insights-sync-worker` service (S1-11, #847). With it off: Conversations 0% positive + empty asks panel, goal `lead_generation` reads 0, attention lead counts 0, top per-post sentiment null, trends sentiment 0 — five surfaces. Flip executed with Hermes creds verified in the **worker** env; post-flip verification per §6 row 1 (watch `insights_sync_runs` + label-review the first classified batch). | flags | XS (ops) |
...
| B4 | **Native reply:** flag OFF. The reply route handler already dispatches to the shipped Composio path (selected when `effectivePublishProvider === 'composio'`; compose default is already `composio`), which needs no App Review. The `/insights` Conversations "Reply" button is a disabled stub ("Reply ships soon" tooltip since S1-7) — the remaining work is UI wiring, not integration. | flags+UI | M |
| B5 | ✅ **Closed 2026-08-05 (S5-4)** — the chip half is done: `#912` derived the chips from `isPlatformInsightsEnabled` (the adapter factory's own predicate) and `#914` removed the dormant TikTok integration, so no chip can exist without an adapter behind it. `tests/insights-platform-filter-truthing.test.ts` pins it both ways and is now in `npm run verify`. The flags themselves remain default OFF by design — that is a rollout decision, not a defect. | flags+UI | S |
...
| # | Gap | Evidence | Effort |
|---|-----|----------|--------|
| F1 | **Weekly Results report + one approved Next Action** — entire 2026-06-01 plan unbuilt (zero code on master). ✅ **F1a rebased 2026-08-02 (S3-4)**: the "insights absent" framing is corrected, best/weakest now ranks over the shipped read model, and the MVP slice is agreed (A/B/C/D.2/F in; D.1/D.3+E deferred behind the B2 repair) — see `docs/plans/2026-06-01-weekly-results-next-action.md`. F1b (the build) is S5-1. | product | XL → **L** after the slice |
...
| F6 | **Two overlapping analytics surfaces in the nav** (`/insights` + `/dashboard/analytics`) — ✅ decision ratified 2026-08-02 (Sprint 1 ticket **S1-10**, not backlog; recorded in this doc's changelog above): `/insights` primary, `/dashboard/analytics` retained as the per-platform drill-down. Further shell investment is unblocked; converging the two surfaces is backlog item 9. | product decision | XS (decision) |
...
| S1-10 | ✅ **Decision ratified 2026-08-02** — surface consolidation (F6): `/insights` primary, `/dashboard/analytics` retained as the per-platform drill-down (not retired) | XS | Written decision recorded in this doc's changelog (2026-08-02 entry); unblocks all later shell investment |
...
| S3-4 | ✅ **Done 2026-08-02** — rebased the 2026-06-01 weekly-results plan against as-built insights (F1a); its "insights absent, degrade" framing was stale, and best/weakest-post now ranks over `insights_post_metrics_daily` via `LATEST_POST_METRICS_LATERAL` | S | Updated plan doc (rebase section + agreed MVP slice: A/B/C/D.2/F in; D.1/D.3+E deferred behind the B2 repair); S5-1 unblocked |
...
| ID | Ticket | Effort | Notes / acceptance |
|----|--------|--------|--------------------|
| S5-1 | Weekly Results MVP (F1b): server-side report builder over shipped builders (summary, trends, top/weakest post, one suggested next action), `/dashboard/results` panel, flag-gated `ARIES_WEEKLY_RESULTS_ENABLED` default OFF. **Scope = the S3-4 MVP slice** (phases A/B/C/D.2/F; the Honcho promotion loop D.1/D.3+E is deferred behind the B2 repair) | L | Live tenant renders a truthful week recap. **Dependencies restated by S3-4:** S2-1 ✅ landed (the builder MUST read metrics through `LATEST_POST_METRICS_LATERAL` — never `SUM` across a post's dated snapshot rows) and S2-3 ✅ landed; S4-1 shipped but flag-default-OFF, so do not assume attribution scoping is active. **Standard flag-landing steps:** `:-0` wiring in the aries-app `environment:` block in docker-compose.yml, `.env.example` entry, CLAUDE.md env-var doc |
...
| S5-4 | ✅ **Done 2026-08-05** — platform filter truthing (B5) had already landed ahead of this sprint in `#912` (chips derived from `isPlatformInsightsEnabled`) + `#914` (TikTok integration removed); verified against the code rather than re-implemented. The residual gap was gating: its test ran only in the CI full suite, so it is now allowlisted in `npm run verify` to catch a regression pre-push | S | Chips match platforms that can actually have data |
...
9. **Execute the F6 consolidation** ratified in S1-10 (changelog entry 2026-08-02) (M) — make `/insights` the single analytics entry point and reach `/dashboard/analytics` from it as the per-platform drill-down. The **route is not deleted**; what changes is its top-level nav entry (`frontend/app-shell/routes.ts`, `id: 'analytics'`) and the drill-down links out of `/insights`. Note `tests/insights-dashboard-ui.test.ts` currently pins both surfaces as top-level `utility` nav routes — it is the test that must move with this ticket.
...
|-------|--------|----------|--------------|--------|
| 1 | ✅ `ARIES_COMMENT_CLASSIFICATION_ENABLED=1` — executed 2026-07-16 | Sprint 1 (S1-11) | Hermes creds present in **aries-insights-sync-worker** env (empty-default trap) | `insights_sync_runs` not `partial` from the classify leg; first batch label-quality-reviewed (labels frozen until S4-3); Conversations sentiment real |
