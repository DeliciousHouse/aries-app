...
- Every dashboard sub-page is `<AppShellLayout currentRouteId="..." loginRedirectPath="...">{<Screen/>}</AppShellLayout>` — see `app/dashboard/results/page.tsx:4-10`. [2026-08-13: AA-229 retired that page to a bare `redirect('/insights')`; any other dashboard sub-page, e.g. `app/dashboard/posts/page.tsx`, still shows the `AppShellLayout` wrapper pattern.] `app/dashboard/layout.tsx` already enforces the onboarding gate for the whole `/dashboard/*` segment (via `enforceOnboardingGate`).
...
**New route `app/dashboard/launch-readiness/page.tsx`** (mirror `app/dashboard/results/page.tsx` [2026-08-13: AA-229 retired that page to a bare `redirect('/insights')` — mirror any other dashboard sub-page's `page.tsx`, e.g. `app/dashboard/posts/page.tsx`, instead]):
...
**New screen `frontend/aries-v1/launch-readiness-screen.tsx`** (`'use client'`, mirror `frontend/aries-v1/post-list.tsx` [2026-08-13: AA-229 retired `results-screen.tsx`; `post-list.tsx` is live and carries the same header-first hydration comment]):
...
- Render the existing shell primitives (`ShellPanel`, `LoadingStateGrid`, `StatusChip` from `./components`) — header renders immediately, content hydrates below (same "looks alive while fetching" pattern `post-list.tsx` documents for the ~10–40s posts fetch).
