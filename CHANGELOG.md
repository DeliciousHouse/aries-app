...
### Changed
- **Tenants with sparse brand profile fields now keep working when the Hermes research stage runs.** Tenants whose Business Profile leaves `business_name` and `notes` blank (the minimum-config path through onboarding) previously sent Hermes a thin request that under-directed the research agent. The agent scraped the site successfully, then looped on `read_file`/`search_files`/`terminal` tool calls until the 600s `did not reach a terminal status` timeout fired — campaigns failed at research with nothing rendered. The auto-extracted `brand-kit.json` already had the right data (`brand_name`, `brand_voice_summary` from the website source review), it just never reached the request payload. Two new `resolve*` helpers in `backend/social-content/workflow-request.ts` close the gap: `resolveBusinessName` falls back to `brand_kit.brand_name` when the operator field is null, and `resolveNotes` falls back to a 300-char-truncated `brand_voice_summary`. The new `notes: string` field on `SocialContentWeeklyBrandPayload` carries this fallback into the request payload Hermes serializes as part of its prompt. Operator-supplied values still win when present.
